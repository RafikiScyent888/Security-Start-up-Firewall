...
/* ---------------------------------------------------------------------
   THE TIER
   --------------------------------------------------------------------- */
export function tierPane(mod, state, opts) {
  opts = opts || {};
  ...
  /* THE SPINE, with the objectives — the same contract as Tier 1.
     Orient, the six graded steps in the order a professional takes
     them, then write it up. The shape repeating across every tier is
     the point: by Tier 5 a student should recognise the method before
     they have read a word of the incident. */
  {
    const ctx = { progress: prog, wroteUp: !!opts.wroteUp };
    const steps = Spine.spineFor(mod.ID);
    const show = Spine.revealed(ctx, opts.spineManual || 1, mod.ID);
    html += `<div class="card">
      <h3>How to work this</h3>
      <p>The order a professional goes at an incident. It does not give you answers &mdash; each
        step is the question to ask next, and the judgment stays yours.</p>
      <p class="lede">Step ${show} of ${steps.length}, and the last one is writing it up.</p>
    </div>`;
    html += steps.slice(0, show).map((st, i) => {
      ...
      return `<div class="objective" data-met="${done}" data-spine-step="${i + 1}">
        ...
        <p class="why"><em>${Spine.labelsFor(st, mod.OBJECTIVES).map(esc).join(" &middot; ")}</em></p>
      </div>`;
    }).join("");
    if (show < steps.length) {
      html += `<div class="card">
        <p>Steps unfold as you finish them &mdash; and you are never held behind one.</p>
        <p><button type="button" class="btn-quiet" data-spine-next-tier="${mod.ID}">Show me the next step</button></p>
      </div>`;
    }
  }
