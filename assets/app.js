...
import * as TierPane from "./tierpane.js";
...
const state = {
  ...
  /* TIERS 2 TO 5. Tier 1 has a world; these have a decision state
     each, kept separately so moving between them loses nothing. */
  activeTier: null,
  tierStates: {},

  /* How many steps of the spine the student has asked to see. Never
     goes backwards, and it is what stops a step they cannot finish
     from hiding the step after it. */
  spineManual: 1,

  /* One counter per tier, because reading ahead in Tier 2 should not
     move the guide in Tier 4. */
  spineManualTier: {}
};

/** The furthest tier the student has actually opened. Panes unlock
    from this, not from a calendar and not from a score — opening Tier
    2 is what makes a business exist, and the business is what makes
    RITSCOM, the tracker and the agreements mean anything. */
function reachedTier() {
  let hi = 1;
  for (const k of Object.keys(state.tierStates || {})) {
    const n = Number(k);
    if (n > hi) hi = n;
  }
  return hi;
...
/** The state for a tier, made on first visit and kept after that. */
function tierState(id) {
  if (!state.tierStates[id]) state.tierStates[id] = Tiers.makeState(id);
  return state.tierStates[id];
}
...
function restore(saved) {
  ...
  /* Tiers 2 to 5 come back with everything else, or a save quietly
     costs somebody four tiers of work. */
  state.tierStates = saved.tierStates || {};
}
...
const TABS = [
  ...
  /* A LITERAL EN DASH, not "&ndash;".

     Every label here goes through esc() before it reaches the page,
     which is correct — a pane label is text, and text that can carry
     markup is a hole. So an entity written here is escaped and the
     student reads "Tiers 2&ndash;5" on the rail. Put the character
     itself in. */
  ["campaign", "Tiers 2–5"],

  /* ON THE RAIL FROM THE FIRST MINUTE, LOCKED UNTIL TIER 2.

     Settled in section 5: "Panes move between tabs as tiers unlock,
     with a note to students explaining when and why." These were left
     out of the build entirely, which is not the same thing. A pane
     that does not exist teaches nothing; a pane that is visibly
     locked tells a student the shape of the job ahead and why they
     cannot do it yet. */
  ["ritscom", "RITSCOM"],
  ["adt", "Action Date Tracker"],
  ["ntk", "NTK — Need to Know"],
...
function renderTabs() {
  ...
  tabs.innerHTML = tabsNow().map(([id, label]) => {
    /* THREE WAYS, never colour alone. The word "Locked" in the rail,
       the data-locked attribute the stylesheet marks, and the full
       note on the pane itself when they open it. And it is never
       dimmed out of readability — a locked label still carries a
       reason the student needs. */
    const shut = Panes.lockedFor(id, reachedTier());
    return `<button type="button" role="tab" data-tab="${id}" aria-selected="${state.tab === id}"
       ${shut ? 'data-locked="true"' : ""} aria-controls="pane">${esc(label)}` +
      (id === "objectives" ? `<span class="badge">${done}/${total}</span>` : "") +
      (shut ? `<span class="badge">Locked</span>` : "") +
      `</button>`;
  }).join("");
...
function renderPane() {
  ...
  if (state.tab === "campaign") html += campaignPane();

  for (const id of ["ritscom", "adt", "ntk"]) {
    if (state.tab !== id) continue;
    const reached = reachedTier();
    if (Panes.lockedFor(id, reached)) { html += Panes.lockNote(id); break; }
    if (id === "ritscom") html += Panes.ritscom(reached, state.tierStates);
    if (id === "adt") html += Panes.adt(reached);
    if (id === "ntk") html += Panes.ntk(reached);
    break;
  }
...
/* ---------------------------------------------------------------------
   ...
   A different shape of tier, and the interface says so rather than
   pretending otherwise. Tier 1 is a network you configure; these are
   situations you decide about. The spine list is the way in, and each
   tier keeps its own working out.
   --------------------------------------------------------------------- */
function campaignPane() {
  if (state.activeTier) {
    const mod = Tiers.tier(state.activeTier);
    if (mod) {
      return `<div class="card"><p><button type="button" class="btn-quiet" data-go-tier="0">
        &larr; All five tiers</button></p></div>` +
        TierPane.tierPane(mod, tierState(mod.ID), {
          spineManual: state.spineManualTier[mod.ID] || 1,
          wroteUp: !!(state.campaign && C.getNotes(state.campaign, mod.ID, "tier").trim())
        });
    }
  }

  const t1done = state.objectives.done(state.world);
  const t1total = state.objectives.total();

  let html = `<div class="card">
    <h2>The whole spine</h2>
    <p>Five tiers, from a home network you inherited to a security operations centre that caught a
      planted insider. The business grows because you take on work you cannot serve at your current
      size &mdash; not because a calendar moved.</p>
    <p class="lede">Tier 1 is a network you configure, with a map, a log and an adversary. Tiers 2
      to 5 are the job after that: situations you have to decide about, where the technical controls
      mostly work and the damage happens anyway. They look different because they are different.</p>
  </div>`;

  html += Tiers.SPINE.map(t => {
    if (t.id === 1) {
      return `<div class="objective" data-met="${t1done === t1total}">
        <div class="head"><span class="state">${t1done}/${t1total}</span>
          <span class="title">Tier 1 &mdash; ${esc(t.name)}</span></div>
        <p class="status">${esc(t.blurb)}</p>
        <p><button type="button" data-go="objectives">Open Tier 1</button></p>
      </div>`;
    }
    const mod = Tiers.tier(t.id);
    const st = tierState(t.id);
    const done = Tiers.doneCount(mod, st);
    const total = (mod.OBJECTIVES || []).length;
    return `<div class="objective" data-met="${done === total}">
      <div class="head"><span class="state">${done}/${total}</span>
        <span class="title">Tier ${t.id} &mdash; ${esc(t.name)}</span></div>
      <p class="status">${esc(t.blurb)}</p>
      <p class="why">${t.months} months of business time.</p>
      <p><button type="button" data-go-tier="${t.id}">Open Tier ${t.id}</button></p>
    </div>`;
  }).join("");
...
function objectivesPane() {
  ...
  let html = `<div class="card">
    ...
    <p class="lede"><strong>This is Tier 1 of five, and all five are playable.</strong>
      Tier 1 is a network you configure. Tiers 2 to 5 are the job after that — situations you have
      to decide about, where the technical controls mostly work and the damage happens anyway.
      They are on the <strong>Tiers 2&ndash;5</strong> pane, and you do not have to finish this one
      first.</p>
  </div>`;

  /* THE SPINE, above the list and with it — settled with the owner so
     the exam labels are reinforced in the same place the work is done.
     Advisory: every step is reachable, none of them blocks anything. */
  {
    /* "Wrote it up" is observable: the campaign record holds the
       student's own words for the tier, so the last step is a thing
       the engine can see rather than a polite suggestion. */
    const ctx = { progress: prog,
                  wroteUp: !!(state.campaign && C.getNotes(state.campaign, 1, "tier").trim()) };
    const show = Spine.revealed(ctx, state.spineManual, 1);
    html += `<div class="card">
      <h3>How to work this</h3>
      <p>The order a technician actually goes at a network they have just walked into. It does
        not give you answers &mdash; each step is the question to ask next, and the judgment stays
        yours.</p>
      <p class="lede">One step at a time, so it is not a wall of text. Step ${show} of
        ${Spine.STEPS.length}, and the last one is writing it up.</p>
    </div>`;

    html += Spine.STEPS.slice(0, show).map((st, i) => {
      const done = Spine.stateOf(st, ctx) === "done";
      return `<div class="objective" data-met="${done}" data-spine-step="${i + 1}">
        <div class="head">
          <span class="state">${done ? "done" : "step " + (i + 1)}</span>
          <span class="title">${esc(st.title)}</span>
        </div>
        <p class="status"><strong>${esc(st.ask)}</strong></p>
        <p class="why">${esc(st.body)}</p>
        <p class="why">${esc(st.why)}</p>
        <p class="why"><em>${Spine.labelsFor(st, null).map(esc).join(" &middot; ")}</em></p>
      </div>`;
    }).join("");

    if (show < Spine.STEPS.length) {
      html += `<div class="card">
        <p>Steps unfold as you finish them &mdash; and you are never held behind one.
          If you are stuck on this step, look at the next one anyway.</p>
        <p><button type="button" class="btn-quiet" data-spine-next>Show me the next step</button></p>
      </div>`;
    }
  }
  ...
  if (done >= prog.length - 1) {
    html += `<div class="card">
      ...
      <p><strong>Tier 2 is next, and it is built.</strong> Six months on, the same house and the
        same network, and a first client — so for the first time there is something here worth
        taking. The camera from tonight is still in it.</p>
      <p><button type="button" class="btn-primary" data-go-tier="2">Go to Tier 2</button>
         <button type="button" data-go="aar">Read the AAR</button>
...
function wire() {
  ...
  document.addEventListener("click", ev => {
    ...
    /* ---- TIERS 2 TO 5 ---------------------------------------------- */
    const goTier = t.closest("[data-go-tier]");
    if (goTier) {
      const id = Number(goTier.getAttribute("data-go-tier"));
      state.activeTier = id > 0 ? id : null;
      state.tab = "campaign";
      state.message = null;
      renderPane();
      window.scrollTo(0, 0);
    ...
    if (dec) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const set = Tiers.setById(mod, dec.getAttribute("data-decide"));
      const r = Dec.pick(tierState(mod.ID), set, dec.getAttribute("data-option"));
      state.message = { ok: r.correct !== false, msg: r.msg };
      C.note(state.campaign, mod.ID, "decision." + set.id,
             (r.correct ? "settled " : "ruled out an option on ") + set.title, {});
      C.save(state.campaign);
      renderPane();
      return;
    }

    const mul = t.closest("[data-multi]");
    if (mul) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const set = Tiers.setById(mod, mul.getAttribute("data-multi"));
      const r = Dec.pickMulti(tierState(mod.ID), set, mul.getAttribute("data-option"));
      state.message = { ok: r.correct !== false, msg: r.msg };
      renderPane();
      return;
    }
    ...
    if (pk) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const r = mod.choose(tierState(mod.ID), pk.getAttribute("data-pick"), pk.getAttribute("data-option"));
      state.message = { ok: true, msg: r.msg };
      renderPane();
      return;
    }

    const hd = t.closest("[data-hint-decide]");
    if (hd) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const set = Tiers.setById(mod, hd.getAttribute("data-hint-decide"));
      /* Asking for help counts as a try. That is what moves the ladder
         for somebody who is thinking rather than clicking, and it is
         the only way a student who guesses nothing ever gets a hint. */
      Dec.ensure(tierState(mod.ID), set.id).attempts += 1;
      renderPane();
      return;
    }

    const hm = t.closest("[data-hint-multi]");
    if (hm) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const set = Tiers.setById(mod, hm.getAttribute("data-hint-multi"));
      Dec.ensureMulti(tierState(mod.ID), set.id).attempts += 1;
      renderPane();
      return;
    }

    const rd = t.closest("[data-reset-decide]");
    if (rd) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const set = Tiers.setById(mod, rd.getAttribute("data-reset-decide"));
      state.message = Dec.reset(tierState(mod.ID), set);
      state.message.ok = true;
      renderPane();
      return;
    }

    const rm = t.closest("[data-reset-multi]");
    if (rm) {
      const mod = Tiers.tier(state.activeTier);
      if (!mod) return;
      const set = Tiers.setById(mod, rm.getAttribute("data-reset-multi"));
      state.message = Dec.resetMulti(tierState(mod.ID), set);
      state.message.ok = true;
      renderPane();
      return;
    }

    const nt = t.closest("[data-spine-next-tier]");
    if (nt) {
      const id = Number(nt.getAttribute("data-spine-next-tier"));
      const total = Spine.spineFor(id).length;
      state.spineManualTier[id] = Math.min(total, (state.spineManualTier[id] || 1) + 1);
      renderPane();
      return;
    }

    if (t.closest("[data-spine-next]")) {
      state.spineManual = Math.min(Spine.STEPS.length, (state.spineManual || 1) + 1);
      renderPane();
      return;
    }
