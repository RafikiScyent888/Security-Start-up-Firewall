...
function objectivesPane() {
  ...
  let html = `<div class="card">
    ...
    <p class="lede"><strong>This is Tier 1 of five, and it is the only tier that exists so far.</strong>
      Tiers 2 to 5 are designed and not built yet, so finishing this one does not open a door to
      another — there is nothing on the far side of it to open. What finishing gives you is the AAR,
      and the other five houses.</p>
  </div>`;
  ...
  if (done >= prog.length - 1) {
    html += `<div class="card">
      ...
      <p><strong>There is no Tier 2 to move on to — it is not built yet.</strong> Tiers 2 to 5 are
        designed on paper and none of them is playable, so nothing here is holding you back and
        there is no button you have failed to find. What is actually next is your AAR, and the
        other five houses, each of which breaks this lesson a different way.</p>
